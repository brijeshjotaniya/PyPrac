...
def BinSrch(n, num_list, steps=0):
	...
	if n == num_list[index]:
		steps += 1
		print(f'Step: {steps}, chkd_index: {index}, value: {num_list[index]}')
		return steps,index
	elif n < num_list[index]:
		steps += 1
		print(f'Step: {steps}, chkd_index: {index}, value: {num_list[index]}')
		steps,tmp_index = BinSrch(n, num_list[l_index:index], steps)
		index -= index
		index += tmp_index
	else:
		steps += 1
		print(f'Step: {steps}, chkd_index: {index}, value: {num_list[index]}')
		steps,tmp_index = BinSrch(n, num_list[index:u_index], steps)
		index += tmp_index
	return steps,index
